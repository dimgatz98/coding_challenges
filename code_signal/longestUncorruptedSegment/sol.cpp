...
vector<int> solution(vector<int> sourceArray, vector<int> destinationArray) {
    int block = 0, max = 0, tmp_block = 0, tmp_max = 0;
    bool change = false;
...
int main() {
    ...
    int tc = 1, n;
    cin >> n;
    vector<int> sourceArray(n), destinationArray(n);
    for (int i = 0 ; i < n ; i++) {
        cin >> sourceArray[i];
    }
    for (int i = 0 ; i < n ; i++) {
        cin >> destinationArray[i];
    }
    // cin >> tc;
    for (int t = 1; t <= tc; t++) {
        // cout << "Case #" << t << ": ";
        cout << solution (sourceArray, destinationArray) << endl;
    }
}
