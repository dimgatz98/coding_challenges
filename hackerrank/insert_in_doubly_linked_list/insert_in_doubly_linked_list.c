...
DoublyLinkedListNode* sortedInsert(DoublyLinkedListNode* llist, int data) {
    if (llist == NULL) {
        return create_doubly_linked_list_node(data);
    }

    if (llist->data >= data) {
        DoublyLinkedListNode *new_head = create_doubly_linked_list_node(data);
        new_head->prev = NULL;
        new_head->next = llist;
        return new_head;
    }
    ...
    DoublyLinkedListNode *save = tmp->next;
    tmp->next = create_doubly_linked_list_node(data);
    tmp->next->next = save;
    tmp->next->prev = tmp;
...
int main()
{
    ...
    for (int t_itr = 0; t_itr < t; t_itr++) {
        ...
        print_doubly_linked_list(llist1, sep);
        printf("\n");
